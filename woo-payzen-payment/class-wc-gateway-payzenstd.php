...
class WC_Gateway_PayzenStd extends WC_Gateway_Payzen {
	...
	public function payment_fields() {
		...
		if ($this->settings['card_data_mode'] == 'MERCHANT') {
			$card_keys = $this->settings['payment_cards'];
	...
	public function save_selected_card($order_id) {
	...
	protected function payzen_fill_request($order) {
		...
		$misc_params = array(
				...
				'contrib' => 'WooCommerce2.x-3.x_1.3.2/' . $version . '/' . PHP_VERSION,
				...
				'ship_to_phone' => str_replace(array('(', '-', ' ', ')'), '', $this->get_order_property($order, 'shipping_phone')),
		...
		if ($locale && PayzenApi::isSupportedLanguage($locale)) {
			...
			$this->payzen_request->set('language', $this->settings['language']);
		}
		...
		$this->payzen_request->set('redirect_enabled', ($this->settings['redirect_enabled'] == 'yes') ? true : false);

		// redirection messages
		$success_message = $this->settings['redirect_success_message'];
		$success_message = isset($success_message[get_locale()]) && $success_message[get_locale()] ? $success_message[get_locale()] : $success_message['en_US'];
		$this->payzen_request->set('redirect_success_message', $success_message);

		$error_message = $this->settings['redirect_error_message'];
		$error_message = isset($error_message[get_locale()]) && $error_message[get_locale()] ? $error_message[get_locale()] : $error_message['en_US'];
		...
		foreach ($config_keys as $key) {
			$this->payzen_request->set($key, $this->settings[$key]);
		}
