...
class WC_Gateway_PayzenMulti extends WC_Gateway_PayzenStd {
	...
	public function init_form_fields() {
		...
		$this->form_fields['payment_options'] = array(
				...
				'type' => 'table',
				'columns' => $columns,
				'validate_fnc' => 'validate_payment_options',
				'description' => $descr
		);
...
<?php
	 ...
	 * @return array
	 */
	public function validate_table_field($key, $value = null) {
		$name = $this->plugin_id . $this->id . '_' . $key;
		$newValue = !is_null($value) ? $value : (key_exists($name, $_POST) ? $_POST[$name] : null);

		if (isset($newValue) && is_array($newValue) && !empty($newValue)) {
			if (method_exists($this, $this->form_fields[$key]['validate_fnc'])) {
				return $this->{$this->form_fields[$key]['validate_fnc']}($newValue);
			} else {
				return $newValue;
			}
		} else {
	...
	public function validate_payment_options($newValue) {
		foreach ($newValue as $code => $option) {
			if (!$option['label']
				...
				unset($newValue[$code]); // not save this option
			} else {
				// clean string
				$newValue[$code] = array_map('esc_attr', array_map('woocommerce_clean', (array) $option));
			}
		}
	...
	private function get_available_options() {
		...
		$options = $this->settings['payment_options'];
		...
		if (count($options) == 1) {
			...
			foreach ($options as $key => $option) {
				echo '<li style="list-style-type: none;">
						...
						<label for="payzenmulti_option_' . $key . '">' . $option['label'] . '</label>
					  </li>';
				$first = false;
			}
		}
